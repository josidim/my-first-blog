...
"TITIA", "TITIO", "TOPOGRAFO", "TROVADOR"]
	...
	# elif csv.Sniffer().has_header(csv_file.read().decode("utf-8")) == True:
		...
		for column in linhas:
			...
			elif (column[1] not in lista_tipos):
				messages.info(request, 'TIPO INVÁLIDO LINHA {} VALOR: {}'.format(linhas.line_num,column[1]))
				return redirect ('upload_csv')
			...
			# elif (len(column[2]) > 30 ):
			...
			elif ( column[2].isalpha()== True and column[2] not in lista_titulos ):
			...
			elif ( column[8].isalpha()== True and column[8] not in lista_complementos ):
				...
				messages.info(request, 'TAMANHO CAMPO VALOR 2 INVÁLIDO LINHA {} VALOR: {}'.format(linhas.line_num,column[11]))
				return redirect ('upload_csv')
				...
				messages.info(request, 'TAMANHO CAMPO VALOR 4 INVÁLIDO LINHA {} VALOR: {}'.format(linhas.line_num,column[15]))
				return redirect ('upload_csv')
				...
				messages.info(request, 'TAMANHO CAMPO VALOR 5 INVÁLIDO LINHA {} VALOR: {}'.format(linhas.line_num,column[17]))
				return redirect ('upload_csv')
